...
func New(ds datasource.DataSource) (*Repository, error) {
	return newRepository(ds)
}

func newRepository(ds datasource.DataSource) (*Repository, error) {
	r := &Repository{
		ds:                 ds,
	...
	if err := ds.SetCallback("contact", r.contactCallback); err != nil {
		return nil, fmt.Errorf("watch contact cache: %w", err)
	}
	if err := ds.SetCallback("chatroom", r.chatroomCallback); err != nil {
		return nil, fmt.Errorf("watch chatroom cache: %w", err)
	}
...
// 表很小（每个企业一行，通常 < 50），全量加载零成本。非企业账户没有映射，
// 此时使用空 cache。
...
func (r *Repository) contactCallback(event fsnotify.Event) error {
	...
	if err := r.initOpenimWordingCache(context.Background()); err != nil {
		log.Warn().Err(err).Msgf("Failed to reinitialize openim wording cache: %s", event.Name)
	}
