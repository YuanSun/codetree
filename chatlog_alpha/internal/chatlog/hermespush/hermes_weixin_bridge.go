...
func sendWeixinViaHermesPython(ctx context.Context, cfg *WeixinConfig, req WeixinSendRequest) error {
...
func discoverHermesPython() (string, string, error) {
	if hermesBin, err := exec.LookPath("hermes"); err == nil {
		if data, err := os.ReadFile(hermesBin); err == nil {
			if pythonBin := pythonFromShebang(string(data)); pythonBin != "" {
				if resolved, err := exec.LookPath(pythonBin); err == nil {
					pythonBin = resolved
				}
...
func pythonFromShebang(script string) string {
...
func ensureHermesBridgeScript() (string, func(), error) {
	return materializeHermesBridgeAsset("hermes-weixin-bridge", hermesWeixinBridgeScript)
}
