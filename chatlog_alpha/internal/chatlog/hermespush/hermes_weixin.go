...
const (
	...
	weixinAppID            = "bot"
	weixinAppClientVersion = (2 << 16) | (2 << 8)
...
func SaveWeixinConfig(input WeixinConfig) (*WeixinConfig, error) {
	...
	if strings.TrimSpace(input.Token) == "" {
		if current, err := DiscoverWeixinConfigAt(home); err == nil {
...
func SendWeixinContext(ctx context.Context, client *http.Client, cfg *WeixinConfig, req WeixinSendRequest) error {
	...
	if len(req.MediaPaths) > 0 {
		if err := sendWeixinViaHermesPython(ctx, cfg, req); err != nil {
			return err
		}
		return nil
	}
	return sendWeixinTextDirect(ctx, client, cfg, req.Text)
}

func sendWeixinTextDirect(ctx context.Context, client *http.Client, cfg *WeixinConfig, text string) error {
	text = strings.TrimSpace(text)
	if text == "" {
		return nil
	}
	contextToken := loadContextToken(cfg.HermesHome, cfg.AccountID, cfg.HomeChannel)
	retCode, errCode, err := sendWeixinTextOnce(ctx, client, cfg, text, contextToken)
	if err == nil {
		return nil
	}
	if contextToken != "" && (retCode == weixinSessionExpired || errCode == weixinSessionExpired) {
		...
		_, _, retryErr := sendWeixinTextOnce(ctx, client, cfg, text, "")
		if retryErr == nil {
			return nil
		}
		return retryErr
...
func sendWeixinTextOnce(ctx context.Context, client *http.Client, cfg *WeixinConfig, text, contextToken string) (int64, int64, error) {
...
func clearContextToken(hermesHome, accountID, chatID string) {
	...
	_ = writePrivateFile(path, append(encoded, '\n'))
}
...
func writePrivateFile(path string, data []byte) error {
	return util.WritePrivateFileAtomic(path, data)
}
