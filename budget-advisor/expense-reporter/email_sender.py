...
class EmailSender:
    ...
    def _convert_monthly_comparison_table(self, text: str) -> str:
        """
        Convert the monthly comparison table from text format to markdown table format.
        This will be converted to HTML by the existing _convert_tables() method.
        """
        import re
        ...
        for i, line in enumerate(lines):
            ...
            if in_comparison_table and 'Category' in line and ('Jan' in line or 'Feb' in line or 'Mar' in line or 'Apr' in line or 'May' in line or 'Jun' in line or 'Jul' in line or 'Aug' in line or 'Sep' in line or 'Oct' in line or 'Nov' in line or 'Dec' in line or 'Change' in line):
                ...
                if header_match:
                    ...
                    result_lines.append(f'| Category | {current_month} | {prev_month} | Change | % |')
                    result_lines.append('|----------|------------|------------|----------|------|')
                continue
            ...
            if in_comparison_table and line.strip() and not all(c in '=-' for c in line.strip()):
                ...
                if line.strip().startswith('TOTAL'):
                    ...
                    for amount in amounts:
                        total_row += f' **{amount.strip()}** |'
                    for change in changes:
                        total_row += f' **{change.strip()}** |'
                    for pct in percentages:
                        total_row += f' **{pct}** |'

                    result_lines.append(total_row)
                    result_lines.append('')
                    in_comparison_table = False
                    continue
                ...
                if dollar_pos > 0:
                    ...
                    if len(amounts) >= 1:
                        row += f' {amounts[0].strip()} |'
                    else:
                        row += ' – |'

                    # Previous month amount
                    if len(amounts) >= 2:
                        row += f' {amounts[1].strip()} |'
                    else:
                        row += ' – |'
    ...
    def _markdown_to_html(self, text: str) -> str:
        ...
        # First, convert monthly comparison table format to markdown
